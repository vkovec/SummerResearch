...
public class Environment implements IEnvironment{
	...
	public Info performOption(String o){
		...
		if(o.equals("up")){
			...
			while(op.isExecutable(currState.getName()) && !op.terminate(currState.getName())){
				...
				if(qValues[currState.getName()][getOptionIndex(o)] < getV(currState.getName())){
					break;
				}
				//
			}
	...
	/**
	 * @param s state
	 * @return the value of interrupting o and selecting a new option/action
	 */
	private double getV(int s){
	...
	/**
	 * @param option
	 * @return the index of the option in the qVal/policy arrays
	 */
	private int getOptionIndex(String option){
		...
		while(keys.hasMoreElements()){
			if(keys.nextElement().equals(option)){
				return i;
			}
			i++;
		}
